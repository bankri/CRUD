...
switch (command) {
  ...
  const index = parseInt(args[1]) - 1; // <- fix di sini
  const updatedText = args.slice(2).join(' ');
  const todosToUpdate = loadTodos();

  if (!todosToUpdate.length) {
    console.log('Belum ada todo yang bisa diperbarui.');
    break;
  }
  ...
  if (!updatedText) {
    console.log('Masukkan todo baru yang ingin diperbarui.');
    break;
  }
  ...
  console.log(`Berhasil mengubah todo ke-${index + 1} dari "${oldTodo}" menjadi "${updatedText}"`);
  break;
}
  ...
  - node index.js update [index] [new task] → Ubah todo
